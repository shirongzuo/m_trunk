    ...
    for k = 1 : length(subFolders)
      ...
      cd(secondPath);
        ...
        for i=1:length(imgNames)
            ...
            im=I(:,:,1);
            im=im(25:end,25:end);
            Dot_valid = dots_process(im,1,intensityThreshold);
%             pause(1);
            ...
            for j=1:length(Dot_valid)
                ...
                Dot_total_above=[Dot_total_above,Dot_valid(j).totalAboveThreshold];
            end
...
%             imgSize = int2str(median(Dot_size));
            imgAboveThres = sum(Dot_total_above);
            ...
            sumAboveThres = sumAboveThres + imgAboveThres;
...
%             record_index = record_index+1;
        ...
        folderAboveThres = int2str(sumAboveThres/itemNumber);
        ...
        intensityThresholdStr = int2str(intensityThreshold);
        
        singleRecordStr = [entry ',' folderName ',' imgName ',' folderDotNum ',' folderDotIntensity ',' dotIntensityStd ',' folderDotSize ',' intensityThresholdStr ',' folderAboveThres];        
        multipleRecord = [multipleRecord, singleRecordStr];
...
fprintf(fileID, 'Entry,Folder,Tested,Name,Number,Intensity,IntensityStd,Size,Threshold,TotalAboveThreshold\n');
